...
@pytest.mark.interpreter
def test_libdevice_acos_interpreter(device):

    @triton.jit
    def kernel(out_ptr, in_ptr, n, BLOCK: tl.constexpr):
        offs = tl.program_id(0) * BLOCK + tl.arange(0, BLOCK)
        mask = offs < n
        x = tl.load(in_ptr + offs, mask=mask)
        tl.store(out_ptr + offs, tl.extra.cuda.libdevice.acos(x), mask=mask)

    size = 128
    x = torch.rand(size, dtype=torch.float32, device=device) * 1.8 - 0.9
    out = torch.empty_like(x)
    kernel[(1, )](out, x, size, BLOCK=size)
    np.testing.assert_allclose(out.cpu().numpy(), np.arccos(x.cpu().numpy()), rtol=1e-5)


@pytest.mark.interpreter
def test_libdevice_sinh_interpreter(device):
    ...
    def kernel(out_ptr, in_ptr, n, BLOCK: tl.constexpr):
        ...
        mask = offs < n
        x = tl.load(in_ptr + offs, mask=mask)
        tl.store(out_ptr + offs, tl.extra.cuda.libdevice.sinh(x), mask=mask)

    size = 128
    x = torch.rand(size, dtype=torch.float32, device=device) * 2.0 - 1.0
    out = torch.empty_like(x)
    kernel[(1, )](out, x, size, BLOCK=size)
    np.testing.assert_allclose(out.cpu().numpy(), np.sinh(x.cpu().numpy()), rtol=1e-5)


@pytest.mark.interpreter
def test_libdevice_atan2_interpreter(device):
    ...
    def kernel(out_ptr, x_ptr, y_ptr, n, BLOCK: tl.constexpr):
        ...
        tl.store(out_ptr + offs, tl.extra.cuda.libdevice.atan2(x, y), mask=mask)

    size = 128
    x = torch.randn(size, dtype=torch.float32, device=device)
    y = torch.randn(size, dtype=torch.float32, device=device) + 0.1
    out = torch.empty_like(x)
    kernel[(1, )](out, x, y, size, BLOCK=size)
    np.testing.assert_allclose(out.cpu().numpy(), np.arctan2(x.cpu().numpy(), y.cpu().numpy()), rtol=1e-5)


@pytest.mark.interpreter
def test_libdevice_pow_interpreter(device):
    ...
    def kernel(out_ptr, x_ptr, y_ptr, n, BLOCK: tl.constexpr):
        ...
        x = tl.load(x_ptr + offs, mask=mask)
        y = tl.load(y_ptr + offs, mask=mask)
        tl.store(out_ptr + offs, tl.extra.cuda.libdevice.pow(x, y), mask=mask)
    ...
    out = torch.empty_like(x)
    kernel[(1, )](out, x, y, size, BLOCK=size)
    np.testing.assert_allclose(out.cpu().numpy(), np.power(x.cpu().numpy(), y.cpu().numpy()), rtol=1e-5)
